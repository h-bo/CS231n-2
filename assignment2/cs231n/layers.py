...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = (x - batch_mean, norm_x, batch_var, gamma, beta, bn_param)
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
    elif mode == 'test':
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    (x_mu, norm_x, batch_var, gamma, beta, bn_param) = cache
    eps = bn_param.get('eps', 1e-5)
    N, D = x_mu.shape
    '''
    out = gamma*xhat + beta
    xhat = (x - batch_mean) / (var + eps)
    batch_mean = np.mean(x, axis=0)
    var = (x - batch_mean)**2
    '''
...
def max_pool_forward_naive(x, pool_param):
    ...
    for n in range(N):
        for c in range(C):
            for h_out in range(H_out):
                for w_out in range(W_out):
                    out[n, c, h_out, w_out] = np.max(x[n, c, \
                        h_out*S:h_out*S + pool_H, w_out*S:w_out*S + pool_W])
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
        for c in range(C):
            for h_out in range(H_out):
                for w_out in range(W_out):
                    pool_patch = x[n, c, h_out*S:h_out*S + pool_H, w_out*S:w_out*S + pool_W]
                    mask = np.zeros(pool_patch.shape)
                    mask[pool_patch==np.max(pool_patch)] = 1
                    dx[n, c, h_out*S:h_out*S + pool_H, w_out*S:w_out*S + pool_W] += \
                        dout[n, c, h_out, w_out] * mask
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    out = np.transpose(out_reshaped.reshape(N, H, W, C), (0, 3, 1, 2))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dx = np.transpose(dx_reshaped.reshape(N, H, W, C), (0, 3, 1, 2))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
