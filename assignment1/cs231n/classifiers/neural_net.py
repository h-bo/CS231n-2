...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    data_loss = np.mean(-np.log(scores_normalized[range(N), y]))
    # scores_prob = scores_exp/np.sum(scores_exp, axis=1, keepdims=True)
    # correct_log_prob = -np.log(scores_prob[range(N), y])
  ...
  def loss_drop(self, X, y=None, reg=0.0, keep=1):
    """
    Compute the loss and gradients for a two layer fully connected neural
    network.
    ...
    z1 = X.dot(W1) + b1
    h1 = relu(z1)
    u1 = (np.random.rand(*h1.shape) < keep) / keep
    h1 *= u1
    scores = h1.dot(W2) + b2
    u2 = (np.random.rand(*scores.shape) < keep) / keep
    scores *= u2
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
  ...
  def train_drop(self, X, y, X_val, y_val,
            learning_rate=1e-3, learning_rate_decay=0.95,
            reg=5e-6, num_iters=100,
            batch_size=200, verbose=False, keep=1):
    """
    Train this neural network using stochastic gradient descent.
    ...
    for it in xrange(num_iters):
      ...
      # Compute loss and gradients using the current minibatch
      loss, grads = self.loss_drop(X_batch, y=y_batch, reg=reg, keep=keep)
      loss_history.append(loss)
